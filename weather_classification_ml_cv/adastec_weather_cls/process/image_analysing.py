...
sun_avg = Average(sun_hga_ftrs_flt)
cloud_avg = Average(cloud_hga_ftrs_flt)
rain_avg = Average(rain_hga_ftrs_flt)
...
cv2.rectangle(img_rect, (530, 770), (650, 840), (0,0,255), 2)

x = [k for k in range(530, 700, 10)]
y = [j for j in range(780, 860 ,10)]
...
img_roi = img_sun.copy()

roi_x = [k for k in range(530, 650, 10)]
roi_y = [j for j in range(770, 840 ,10)]

for i in range(len(x)-1):
    for j in range(len(y)-1):
        ort = int(img_roi[x[i]:x[i+1], y[j]:y[j+1]].mean())
        roi_ort.append(ort)

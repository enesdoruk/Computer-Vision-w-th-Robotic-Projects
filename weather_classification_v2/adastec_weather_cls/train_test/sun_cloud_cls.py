...
def train(dataset_path, size_x, size_y, preprocess, test_size,
        ...
        for i in range(len(image_dir)):
            ...
            if image != 'None':
                ...
                if dataset_folder[j] == 'Sunny':
                    global_feature = np.hstack([features_hga, features_hsv, features_gabor, 0])
                elif dataset_folder[j] == 'Cloudy':
                    global_feature = np.hstack([features_hga, features_hsv, features_gabor, 1])
                elif dataset_folder[j] == 'Rainy':
                    global_feature = np.hstack([features_hga, features_hsv, features_gabor, 2])
    ...
    if preprocess == True: 
        ...
        sun_cloud = dataset.sun_cloud_dataset(dataset_pre)
        #dataset.list2csv(cloud_rain, file_name= '/home/enesdrk/Desktop/xx.csv')
    
    if preprocess == False:
        dataset = data_preprocess(total_list)
        sun_cloud = dataset.sun_cloud_dataset(total_list)
...
def get_args():
    ...
    parser.add_argument('-sp', '--save_path', type=str, 
                        default= os.path.expanduser('~') + '/Desktop/weather_classification/adastec_weather_cls/weights/sun_cloud_wghts_adabostDT.sav',

...
class feature_extract:
    ...
    def calc_hga(self, image):
        ...
        for j in range(grad.shape[0]):
            for k in range(grad.shape[1]):
                hga_flat.append(grad[j,k])
        
        avg_hga = self.average(hga_flat)
    ...
    def calc_HSV(self, image):
        ...
        avg_hsv = (self.average(h_list) + self.average(s_list) + self.average(v_list)) / 3
        
        return features_flatten, avg_hsv
    ...
    def calc_roi(self, image):
        ...
        avg_roi = self.average(roi_ort)

        return roi_ort, avg_roi
